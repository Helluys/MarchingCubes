...
namespace Syulleh.MarchingCubes {
	public class MarchingCubesUnitWizard : ScriptableWizard {
		public Material material;

		[MenuItem("Syulleh/Marching Cubes - units")]
		static void DisplayMCWizard () {
			DisplayWizard<MarchingCubesUnitWizard>("Marching  - units");
		}

		private void OnWizardCreate () {
			Debug.Log("Generating " + configurations.Count() + " cubes...");
			int i = 0;
			foreach (CubeMesh cube in configurations.Values) {
				UnityMesh uMesh = new() {
