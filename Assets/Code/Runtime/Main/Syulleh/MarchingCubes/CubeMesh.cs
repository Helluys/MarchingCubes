...
namespace Syulleh.MarchingCubes {
	...
	public class CubeMesh : IEquatable<CubeMesh> {
		...
		public CubeMesh (int[] populatedVertices, int[] populatedEdges, Vector3i[] triangles) {
		...
		public CubeMesh Map (Func<int, int> mapVertex, Func<int, int> mapEdge) {
			Vector3i mapTri (Vector3i v) => new(mapEdge(v.x), mapEdge(v.y), mapEdge(v.z));
		...
		public CubeMesh Reverse () {
			static Vector3i reverseTri (Vector3i v) => new(v.x, v.z, v.y);
